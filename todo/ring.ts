...
export class ring_t<R> {
  ...
  get zero(): R {
    return this.addition.id
  }
  add(x: R, y: R): R {
    return this.addition.add(x, y)
  }
  neg(x: R): R {
    return this.addition.neg(x)
  }
  sub(x: R, y: R): R {
    return this.addition.sub(x, y)
  }

  get one(): R {
    return this.multiplication.id
  }
  mul(x: R, y: R): R {
    return this.multiplication.mul(x, y)
  }

  eq(x: R, y: R): boolean {
    return this.elements.eq(x, y)
  }
...
export class commutative_ring_t<R> extends ring_t<R> {
  commu(x: R, y: R) {
    eqv(this.elements, this.mul(x, y), this.mul(y, x))
  }
...
export class integral_ring_t<R> extends commutative_ring_t<R> {
  nonzero_product(x: R, y: R) {
    not_eqv(this.elements, x, this.zero)
    not_eqv(this.elements, y, this.zero)
    not_eqv(this.elements, this.mul(x, y), this.zero)
  }
}

export class euclidean_ring_t<R> extends integral_ring_t<R> {
  ...
  euclidean_divmod(x: R, y: R) {
    ...
    assert(this.eq(r, this.zero) || this.degree_lt(r, y))
  }
  ...
  gcd_ext(x: R, y: R): [R, [R, R]] {
    ...
    while (!this.zero_p(r)) {
      if (this.degree_lt(old_r, r)) {
        ;[s, t, r, old_s, old_t, old_r] = [old_s, old_t, old_r, s, t, r]
      } else {
  ...
  gcd_ext_p(x: R, y: R, res: [R, [R, R]]): boolean {
    let [d, [s, t]] = res
    return (
      this.eq(d, this.gcd(x, y)) &&
      this.eq(d, this.add(this.mul(s, x), this.mul(t, y)))
    )
  }
}

export class field_t<R> extends integral_ring_t<R> {
  inv: (x: R) => R
