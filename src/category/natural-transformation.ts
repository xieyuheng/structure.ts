...
export class natural_transformation_t<D, DM, C, CM> {
  dom: category_t<D, DM>
  cod: category_t<C, CM>
  ...
  constructor(the: {
    ...
    this.tar = the.tar
    this.component = (x) => {
      the_eqv(this.cod.objects, this.cod.dom(the.component(x)), this.src.map(x))
      the_eqv(this.cod.objects, this.cod.cod(the.component(x)), this.tar.map(x))
  ...
  naturality(f: DM) {
    ...
    the_eqv(
      this.cod.morphisms,
      this.cod.compose(this.component(a), this.tar.fmap(f)),
      this.cod.compose(this.src.fmap(f), this.component(b))
    )
  }
}
