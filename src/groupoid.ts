...
export class groupoid_t<O, M> extends category_t<O, M> {
  inv: (f: M) => M
  ...
  morphism_iso(f: M) {
    the_eqv(this.morphisms, this.compose(f, this.inv(f)), this.id(this.dom(f)))

    the_eqv(this.morphisms, this.compose(this.inv(f), f), this.id(this.cod(f)))
  }
}
